...
export default async function BatchDetailPage({
  ...
  const currentStudentIds = batch.studentBatches.map(sb => sb.student.id)
          ...
          <Card className="h-fit">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-sm">Batch ID: {batch.id}</p>
              </div>
              <div>
                  ...
                  batch.examBatches.map((eb) => (
                    <div key={eb.exam.id} className="p-2 rounded-md border bg-muted/20">
                      ...
                      <Button variant="link" size="sm" asChild className="h-auto p-0 mt-2 text-xs">
                        <Link href={`/admin/exams/${eb.exam.id}`}>View Exam Details</Link>
                      </Button>
                    </div>
                  ))
                )}
